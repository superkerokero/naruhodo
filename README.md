...
* ### Improvement on the quality of generated graph (0.2 ~ 0.5)
    ...
    As you can see from the source code, naruhodo relies on rule-based system to parse given information exclusively.
    And for a subject as large and complex as a language, long-term testing and procedural improvement is neccessary before it can go anywhere.
...
* ### Support for other languages(?~)
    ...
    Japanese is the only language naruhodo supports now. Besides my personal interest, I chosen Japanese because of it has some unique characteristics that make it challenging and subsequencially rewarding as well. In my opnion, the difficulty regarding Japanese mostly comes from its ambiguity in the expression(for example, the subject is frequently ommited in Japanese) and large amount of word transformations(the same verb can have as many as 10+ forms).

    From a practical point of view, languages such as English and Chinese are in potentially larger demand. So I am thinking about expanding the library to these popular languages in the future, if the rule-based approach taken by naruhodo proves to be attractive.
