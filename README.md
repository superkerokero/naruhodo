...
The directed linking nature of dependency grammar makes the result of dependency parsing directed graphs. This dependency information, combined with the information of word types and transformations, are the bread and butter of the text comprehension(or assimilation of information) by computers instead of humans.

Dependency structure graph(DSG) is not immediately useful to humans or computers(except for getting some rough ideas about the keywords and complexity of your input text), but it is important information and can be used to generate knowledge structure graph(KSG) that will be introduced later. 
...
Unlike DSG, knowledge structure graph(KSG) tries to capture the meaningful relationship between different entities. KSG is generated using the output of dependency parsing program with other information like word types and transformations.
...
I think this kind of graph structure is very close to how information/knowledge is stored in human brains. The goal of naruhodo's KSG parsing is to generate this kind of graph structure from human readable texts automatically as accurate and complete as possible.
...
* ### Applications based on DSG and KSG(new projects)
    ...
    * knowledge database for Q&A system and translation system 
    * sentiment analysis on specific entity
